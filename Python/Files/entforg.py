...
def create_diagonal_circuits(n_qubits_site,k,sort_index):
    # Function that creates |b_n> circuits
    diagonal_circuits = [0] * k

    for i in range(k):
        if sort_index is None:
            bin_string = bin(i)[2:]
        else:
            bin_string = bin(int(sort_index[i]))[2:]
        qc = QuantumCircuit(n_qubits_site)
        for l, j in enumerate(reversed(bin_string)):
            if j == '1':
                qc.x(l)
...
def create_phi_circuit(qc, x, y, p):
    ...
    for l in diff:
        if l != k:
            qc.cx(len(x)-1-k, len(x)-1-l)
    return qc

def create_off_diagonal_circuits(n_qubits_site,k,sort_index):
    # Create all phi circuits
    format_str = '{0:0' + str(n_qubits_site) +'b}'

    phi_circuits = [0] * (2*(k**2-k))
    ...
    for n in range(k):
        if sort_index is None:
            ...
            bn = format_str.format(int(sort_index[n]))
        for m in range(n):
            if sort_index is None:
                bm = format_str.format(m)
            else:
                bm = format_str.format(int(sort_index[m]))
...
def expectation_h_tot(params, beta, diagonal_circuits_U, phi_circuits_U, H_q, H_int_q, estimator):
    ...
    diagonal_circuits_to_measure_U = [circuit.assign_parameters(theta_par) for circuit in diagonal_circuits_U]
    phi_circuits_to_measure_U      = [circuit.assign_parameters(theta_par) for circuit in phi_circuits_U]
    
    diagonal_circuits_to_measure_V = [circuit.assign_parameters(-theta_par) for circuit in diagonal_circuits_U]
    phi_circuits_to_measure_V      = [circuit.assign_parameters(-theta_par) for circuit in phi_circuits_U]

    k=len(diagonal_circuits_to_measure_U)
    # First compute the energy of H_L
    # We only have to compute diagonal terms
    sampled_energies = measure(H_q, diagonal_circuits_to_measure_U, estimator)
    ...
    for H_int_term in H_int_q.to_list():
        ...
        if pauli_string == 'I'*n_qubits_total:
            ...
            O1 =SparsePauliOp(pauli_string[:n_qubits_site])
            O2 =SparsePauliOp(pauli_string[n_qubits_site:])

            # Compute all expectation values needed
            O1_exps = measure(O1, diagonal_circuits_to_measure_U + phi_circuits_to_measure_U, estimator)
            O2_exps = measure(O2, diagonal_circuits_to_measure_V + phi_circuits_to_measure_V, estimator)
            
            # Diagonal terms
            diagonal_O1_exps = O1_exps[:k]
            diagonal_O2_exps = O2_exps[:k]

            # Phi terms
            off_diagonal_O1_exps = O1_exps[k:]
            off_diagonal_O2_exps = O2_exps[k:]
            ...
            arr = np.zeros((k, k), dtype='complex')
...
def measure(op, circuits, estimator):
    m=len(circuits)
    Energies=np.zeros(m)
    for i in range(m):
        job = estimator.run(circuits[i],op)
